'use client'
import { Avatar, Badge, Button, Card, Modal, PageBanner, SearchBar } from '@/components/ui'
import { Calendar, Check, Edit, Eye, Grid, Heart, List, MessageSquare, Plus, Search, Trash2 } from 'lucide-react'
...
export default function BlogPostsPage() {
  ...
  const stats = [
    ...
    { l:'Total Views',  v:posts.reduce((a,p) => a+p.views, 0).toLocaleString('en-US'),   c:'var(--secondary)'},
  ...
  return (
    <>
      ...
      <PageBanner title="Blog Posts" breadcrumbs={[{label:'Home',href:'/'},{label:'Blog'},{label:'Posts'}]}
        action={<Link href="/apps/blog/create"
        description="Manage and moderate all blog posts"><Button><Plus size={14}/>New Post</Button></Link>}/>
            ...
            <div key={p.id} className="card group overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 text-xs pt-3 border-t" style={{color:'var(--muted)',borderColor:'var(--border)'}}>
                  <span className="flex items-center gap-1"><Eye size={11}/>{p.views.toLocaleString('en-US')}</span>
                  ...
                  <div className="ml-auto flex gap-1">
                    ...
                    <Link href="/apps/blog/edit"><button className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Edit size={12}/></button></Link>
                    <button onClick={()=>setDeleteId(p.id)} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>{filtered.map(p=>(
                <tr key={p.id} className="border-b hover:bg-[var(--surface)] transition-colors cursor-pointer" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                  ...
                  <td className="px-5 py-3.5 text-sm" style={{color:'var(--muted)'}}>{p.views.toLocaleString('en-US')}</td>
                  <td className="px-5 py-3.5"><Badge variant={STA_V[p.status]}>{p.status}</Badge></td>
                  <td className="px-5 py-3.5"><div className="flex gap-1">
                    <button className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Eye size={12}/></button>
                    <Link href="/apps/blog/edit"><button className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Edit size={12}/></button></Link>
                    <button onClick={()=>setDeleteId(p.id)} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>
                  </div></td>
                </tr>
