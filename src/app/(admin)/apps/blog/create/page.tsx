...
import { AlignCenter, AlignLeft, AlignRight, Bold, Check, Code, Image, Italic, Link2, List, Plus, Upload, X } from 'lucide-react'
...
export default function BlogCreatePage() {
  ...
  if (done) {
    return (
      <>
        ...
        description="Write and publish new blog content"><Button variant="outline" onClick={()=>{setSaving(true);setTimeout(()=>{setSaving(false)},800)}}>Save Draft</Button><Button loading={saving} onClick={()=>{setSaving(true);setTimeout(()=>{setSaving(false);setDone(true)},1000)}}><Check size={14}/>Publish</Button></div>}/>
  ...
  return (
    <>
      <PageBanner title="Create Post" breadcrumbs={[{label:'Home',href:'/'},{label:'Blog'},{label:'Posts',href:'/apps/blog/posts'},{label:'Create'}]}
        action={<div className="flex gap-2"><Button variant="outline" onClick={()=>{setSaving(true);setTimeout(()=>{setSaving(false)},800)}}>Save Draft</Button><Button loading={saving} onClick={()=>{setSaving(true);setTimeout(()=>{setSaving(false);setDone(true)},1000)}}><Check size={14}/>Publish</Button></div>}/>
