...
import { Toast, PageBanner, Card, CardHeader, Badge, Button, Modal, Input, Progress } from '@/components/ui'
      ...
      <Card padding={false}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {sales.map(s=>(
                <tr key={s.id} className="border-b hover:bg-[var(--surface)] transition-colors cursor-pointer" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-1">
                      ...
                      <button type="button" onClick={()=>{setSales(ss=>ss.filter(x=>x.id!==s.id));showToast('Deleted')}} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Trash2 size={13}/></button>
                    </div>
                  </td>
                </tr>
              ))}
