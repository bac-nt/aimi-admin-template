 ...
 */
import React, { Alert, Badge, Button, Card, CardHeader, Input, Modal, PageBanner, Progress, SearchBar, Tabs, Toggle } from '@/components/ui'
import {
...
const INIT_USERS: IAMUser[] = [
  {id:1,name:'Mathew Anderson',email:'admin@aimi.com',  role:'super_admin',status:'active',    lastLogin:'Just now',   mfa:true, sessions:2},
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(MODULES).map(([key,mod])=>{
                    ...
                    return (
                      <tr key={key} className="border-b hover:bg-[var(--surface)] transition-colors cursor-pointer" style={{borderColor:'rgba(232,237,242,0.5)'}}>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {filtered.map(u=>{
                  ...
                  return (
                    <tr key={u.id} className="border-b hover:bg-[var(--surface)] transition-colors cursor-pointer" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                      ...
                      <td className="px-5 py-3.5"><div className="flex gap-1">
                        <button type="button" className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Edit size={12}/></button>
                        {u.status!=='suspended'
                        ...
                        {u.role!=='super_admin'&&<button type="button" onClick={()=>setUsers(us=>us.filter(x=>x.id!==u.id))} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-all hover:scale-105" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>}
                      </div></td>
                    </tr>
                  )
                })}
  ...
  return (
    <>
      <PageBanner title="IAM & Access" breadcrumbs={[{label:'Home',href:'/'},{label:'Administration'},{label:'IAM & Access'}]} description="Roles, permissions, members and security — all in one place"
        action={<Button onClick={()=>setInvMod(true)}><Plus size={14}/>Invite Member</Button>}/>
