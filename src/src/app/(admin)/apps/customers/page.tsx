...
import { PageBanner, Card, Badge, Button, Modal, Input, Select, Progress } from '@/components/ui'
    ...
    <>
      <PageBanner title="Customers" breadcrumbs={[{label:'Home',href:'/'},{label:'Ecommerce'},{label:'Customers'}]}
        ...
        action={<div className="flex gap-2">
          ...
          <Button size="sm" onClick={()=>setInvModal(true)}><Plus size={14}/>Add Customer</Button>
        </div>}
      />
      ...
      <Card padding={false}>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 border-b" style={{borderColor:'var(--border)'}}>
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border flex-1 max-w-xs" style={{background:'var(--surface)',borderColor:'var(--border)'}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search customers…" className="bg-transparent text-sm outline-none w-full" style={{color:'var(--foreground)'}}/>
          </div>
                  ...
                  <tr key={c.id} className="border-b hover:bg-[var(--surface)] transition-colors cursor-pointer" style={{borderColor:'rgba(232,237,242,0.7)'}} onClick={()=>setDetail(c)}>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1" onClick={e=>e.stopPropagation()}>
                        ...
                        {c.tier!=='Platinum' && <button type="button" onClick={()=>{setCustomers(cs=>cs.filter(x=>x.id!==c.id));showToast('Customer removed')}} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={13}/></button>}
                      </div>
                    </td>
                  </tr>
                )
