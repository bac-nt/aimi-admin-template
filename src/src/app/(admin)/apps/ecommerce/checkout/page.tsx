...
import { PageBanner, Card, CardHeader, Badge, Button, Toggle, Input, Select } from '@/components/ui'
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        {/* Payment Gateways */}
        <Card>
          <CardHeader title="Payment Gateways" subtitle="Enable and configure payment processors" action={<Button size="sm" variant="outline"><Plus size={12}/>Add</Button>}/>
            ...
            {gateways.map(g=>(
              <div key={g.id} className="p-4 rounded-xl border transition-all" style={{borderColor:g.enabled?'var(--primary)44':'var(--border)',background:g.enabled?'var(--primary-light)':'transparent'}}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Toggle checked={g.enabled} onChange={v=>setGateways(gs=>gs.map(x=>x.id===g.id?{...x,enabled:v}:x))} size="sm"/>
                </div>
                {g.enabled && (
