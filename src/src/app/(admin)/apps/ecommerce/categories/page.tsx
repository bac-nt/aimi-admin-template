...
import { PageBanner, Card, Badge, Button, Modal } from '@/components/ui'
    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {[
          ...
          {l:'Total Products',v:cats.reduce((a,c)=>a+c.count,0).toLocaleString(),c:COLOR.warning},
        ].map(s=>(
          ...
          <Card padding={false}>
            <div className="p-4 border-b flex items-center gap-3" style={{borderColor:'var(--border)'}}>
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg border flex-1 max-w-xs" style={{background:'var(--surface)',borderColor:'var(--border)'}}>
                <Search size={14} style={{color:'var(--muted)'}}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search categories…" className="bg-transparent text-sm outline-none w-full" style={{color:'var(--foreground)'}}/>
              </div>
              <Button onClick={openCreate} className="ml-auto"><Plus size={14}/>Add Category</Button>
            </div>
                ...
                <tbody>
                  {(search ? filtered : topLevel).map(cat => (
                    <React.Fragment key={cat.id}>
                      <tr className="border-b hover:bg-[var(--surface)] transition-colors" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                        ...
                        <td className="px-5 py-3.5"><div className="flex gap-1">
                          <button onClick={()=>openEdit(cat)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-colors" style={{color:'var(--muted)'}}><Edit size={12}/></button>
                          <button onClick={()=>setCats(c=>c.filter(x=>x.id!==cat.id))} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>
                        </div></td>
                      </tr>
                      {!search && children(cat.id).map(child=>(
                        <tr key={child.id} className="border-b hover:bg-[var(--surface)] transition-colors" style={{borderColor:'rgba(232,237,242,0.7)',background:'rgba(93,135,255,0.02)'}}>
                          ...
                          <td className="px-5 py-3"><div className="flex gap-1">
                            <button onClick={()=>openEdit(child)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-colors" style={{color:'var(--muted)'}}><Edit size={12}/></button>
                            <button onClick={()=>setCats(c=>c.filter(x=>x.id!==child.id))} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>
                          </div></td>
                        </tr>
                      ))}
                    </React.Fragment>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="field w-full" value={form.parent} onChange={e=>setForm(f=>({...f,parent:e.target.value}))}>
