        ...
        <div className="xl:col-span-2 space-y-5">
          <Card>
            <div className="flex items-center gap-3 mb-4">
              ...
              />
              <Button onClick={openCreate}><Plus size={14}/>Add Tag</Button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>{filtered.map(tag => (
                  <tr key={tag.id} className="border-b hover:bg-[var(--surface)] transition-colors" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                    ...
                    <td className="px-5 py-3.5"><div className="flex gap-1">
                      <button onClick={() => openEdit(tag)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--primary-light)] hover:text-[var(--primary)] transition-colors" style={{color:'var(--muted)'}}><Edit size={12}/></button>
                      <button onClick={() => setTags(ts => ts.filter(t => t.id !== tag.id))} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={12}/></button>
                    </div></td>
                  </tr>
                ))}</tbody>
              </table>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            {[
              ...
              {l:'Total Uses',v:tags.reduce((a,t)=>a+t.count,0).toLocaleString()},
