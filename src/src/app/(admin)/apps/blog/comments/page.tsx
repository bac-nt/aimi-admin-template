      ...
      <PageBanner title="Comments" breadcrumbs={[{label:'Home',href:'/'},{label:'Blog'},{label:'Comments'}]}
        ...
        action={
          <div className="flex gap-2">
            {counts.Pending>0 && (
              <Button size="sm" variant="success" onClick={()=>{comments.filter(c=>c.status==='Pending').forEach(c=>approve(c.id));showToast(`${counts.Pending} comments approved`)}}>
      ...
      <Card padding={false}>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 border-b" style={{borderColor:'var(--border)'}}>
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border flex-1 max-w-xs" style={{background:'var(--surface)',borderColor:'var(--border)'}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search comments…" className="bg-transparent text-sm outline-none w-full" style={{color:'var(--foreground)'}}/>
          </div>
