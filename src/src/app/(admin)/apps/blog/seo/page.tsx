'use client'
import { PageBanner, Card, Badge, Button, Progress } from '@/components/ui'
import { Search, TrendingUp, Globe, Link, AlertCircle, Check, ExternalLink } from 'lucide-react'
          ...
          <Card>
            ...
            <div className="space-y-3">
              {[
                ...
                {icon:Link,text:'Build internal links between related posts',color:COLOR.warning,done:false},
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>{SEO_POSTS.map(p=>(
                <tr key={p.id} className="border-b hover:bg-[var(--surface)] transition-colors" style={{borderColor:'rgba(232,237,242,0.7)'}}>
                  ...
                  <td className="px-5 py-3.5 font-semibold" style={{color:'var(--foreground)'}}>{p.impressions.toLocaleString()}</td>
                  <td className="px-5 py-3.5 font-semibold" style={{color:'var(--foreground)'}}>{p.clicks.toLocaleString()}</td>
