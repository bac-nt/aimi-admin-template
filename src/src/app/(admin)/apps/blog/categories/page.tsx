            ...
            <Card key={cat.id} padding={false}>
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button type="button" onClick={()=>{setCats(cs=>cs.filter(c=>c.id!==cat.id&&c.parent!==cat.name));showToast('Deleted')}} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={13}/></button>
                </div>
              </div>
              {childOf(cat.name).map(child => (
                <div key={child.id} className="flex items-center gap-3 px-5 py-3 border-t ml-8" style={{borderColor:'var(--border)'}}>
                  ...
                  <button type="button" onClick={()=>{setCats(cs=>cs.filter(c=>c.id!==child.id));showToast('Deleted')}} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--error-light)] hover:text-[var(--error)] transition-colors" style={{color:'var(--muted)'}}><Trash2 size={13}/></button>
                </div>
              ))}
            </Card>
          ))}
